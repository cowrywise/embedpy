...
def test_can_get_single_asset(mock_get_essential_details, api_session):
    asset = Asset(api_session)
    mock_get_essential_details.return_value = MagicMock(data=json_responses.GET_SINGLE_ASSET_RESPONSE, status_code=200)
    test_data = {"asset_id": "fake-asset-id"}
    response = asset.get_asset(
        asset_id=test_data.get("asset_id")
    )
    assert response.data == json_responses.GET_SINGLE_ASSET_RESPONSE and response.status_code == 200
    asset.get_essential_details.assert_called_with(
        "GET",
